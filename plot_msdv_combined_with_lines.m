% filepath: plot_msdv_combined_with_lines.m
% Purpose: Plot MSDV (Frequency Domain) vs Time for multiple distances on a single graph
% This version includes connecting lines between points for better trend visualization
% Creates a thesis-ready combined plot showing trends with increasing distance
...
line_width = 1.5;     % Line width for connecting lines
...
% ========================================================================
% PLOT EACH DISTANCE WITH LINES
% ========================================================================
for i = 1:length(distances)
    len = distances{i};
    comp_col = ['Comp_' len];  % Frequency Domain column
    
    % Plot with distinct color, marker, and connecting line
...
print(fig, fullfile(outDir, 'MSDV_FreqDomain_vs_Time_Combined_WithLines.png'), '-dpng', '-r300');

% Save PDF (optimized for thesis documents)
set(fig, 'PaperPositionMode', 'auto');
set(fig, 'PaperUnits', 'inches');
set(fig, 'PaperSize', [14 5.5]);
print(fig, fullfile(outDir, 'MSDV_FreqDomain_vs_Time_Combined_WithLines.pdf'), '-dpdf', '-r300', '-painters', '-fillpage');

% Save EPS for publication quality
print(fig, fullfile(outDir, 'MSDV_FreqDomain_vs_Time_Combined_WithLines.eps'), '-depsc', '-r300', '-painters');

fprintf('\n=== Combined MSDV Plot (With Lines) Generated Successfully ===\n');
fprintf('Output files saved in: %s\n', outDir);
fprintf('  - MSDV_FreqDomain_vs_Time_Combined_WithLines.png\n');
fprintf('  - MSDV_FreqDomain_vs_Time_Combined_WithLines.pdf\n');
fprintf('  - MSDV_FreqDomain_vs_Time_Combined_WithLines.eps\n');
